...
try:
    ...
    raise SystemExit(
        "Fluent UI dependencies are required:\n"
...
STATUS_META = {
    "ready": ("Ready", "#475569", "#E2E8F0"),
    "running": ("Downloading", "#166534", "#DCFCE7"),
    "stopping": ("Stopping", "#92400E", "#FEF3C7"),
...
class FluentDownloader(QWidget):
    ...
    def _build_ui(self):
        ...
        self.subtitle_label = CaptionLabel(
            "Some videos require authorization. Cookies are needed for that.\n"
            "1) launch Edge for cookies  2) sign in  3) click \"Refresh cookies from Edge\"  4) verify the path to cookies.txt",
            self,
        )
        ...
        self.pick_dir_button = PushButton("Browse", self)
        self.open_dir_button = PushButton("Open", self)
        self.pick_cookies_button = PushButton("Browse", self)
        self.refresh_cookies_button = PushButton("Refresh cookies from Edge", self)
        self.launch_edge_button = PushButton("Launch Edge for cookies", self)
        self.check_deps_button = PushButton("Check dependencies", self)
        self.save_settings_button = PushButton("Save settings", self)
        for button in (
        ...
        form_layout.addWidget(BodyLabel("Link", self), 0, 0, alignment=Qt.AlignVCenter | Qt.AlignLeft)
        ...
        form_layout.addWidget(BodyLabel("Download folder", self), 1, 0, alignment=Qt.AlignVCenter | Qt.AlignLeft)
        ...
        form_layout.addWidget(BodyLabel("Cookies file", self), 2, 0, alignment=Qt.AlignVCenter | Qt.AlignLeft)
        ...
        cookies_row.addWidget(BodyLabel("Port", self))
        ...
        format_row.addWidget(BodyLabel("Format", self))
    ...
    def _build_history_panel(self):
        ...
        layout.setSpacing(8)
        title = StrongBodyLabel("Downloaded files", self)
    ...
    def _build_log_panel(self):
        ...
        top.addStretch(1)
        self.clear_log_button = PushButton("Clear log", self)
        self.copy_log_button = PushButton("Copy log", self)
    ...
    def copy_log(self):
        ...
        self.write_log("Log copied to clipboard.")
    ...
    def on_error(self, text: str):
        ...
        self.set_log_color("ERROR: " + text, "#B91C1C")
    ...
    def open_download_folder(self):
        ...
        if not path.exists():
            QMessageBox.critical(self, core.APP_NAME, f"Folder not found: {path}")
            return
    ...
    def open_history_link(self, item: dict[str, str]):
        ...
        if not url:
            QMessageBox.warning(self, core.APP_NAME, "No source link was saved for this file.")
            return
        webbrowser.open(url)
    ...
    def delete_history_file(self, item: dict[str, str]):
        path = Path(item["path"])
        if path.exists():
            prompt = f"Delete file?\n{path.name}"
        else:
            prompt = (
                "The file is already missing on disk.\n"
                f"Remove the entry from the list?\n{path}"
            )
        ...
        if path.exists():
            try:
                path.unlink()
                self.write_log(f"Deleted file: {path}")
            except Exception as exc:
                QMessageBox.critical(self, core.APP_NAME, f"Could not delete the file: {exc}")
                return
    ...
    def refresh_dependency_status(self):
        ...
        self.deps_label.setText(
            f"yt-dlp: {'found' if ytdlp_found else 'not found'} | "
    ...
    def check_dependencies(self):
        ...
        if missing:
            ...
            QMessageBox.warning(
                ...
                + missing_text
                + ".\n\nInstall them in the system or place them next to the app.",
            )
            self.write_log("Dependency check: missing " + missing_text)
            return
        ...
        self.write_log("Dependency check: yt-dlp and ffmpeg were found.")

    def launch_edge(self):
        edge_path = core.find_edge_path()
        if not edge_path:
            QMessageBox.critical(self, core.APP_NAME, "msedge.exe was not found.")
            return
        ...
        self.write_log(f"Edge was launched for cookie capture on port {port}.")

    def refresh_cookies(self):
        if self.cookies_thread and self.cookies_thread.is_alive():
            QMessageBox.information(self, core.APP_NAME, "Cookie refresh is already running.")
            return
        self.persist_settings(silent=True)
        ...
        def runner():
            try:
                self.bus.log.emit(f"Refreshing cookies into {output_path}")
                result = core.export_cookies_from_edge(output_path, port)
                count = result["written"]
                if result["authorized"]:
                    self.bus.info.emit(f"Cookies updated. Authorization detected. Lines written: {count}")
                else:
                    self.bus.info.emit(
                        f"Cookies were saved, but authorization was not detected. "
                        f"Make sure you are signed in inside Edge. Lines written: {count}"
                    )
            except Exception as exc:
    ...
    def validate_inputs(self):
        if not self.url_edit.text().strip():
            QMessageBox.warning(self, core.APP_NAME, "Enter a video or playlist link.")
            return False
        if not self.cookies_path_edit.text().strip():
            QMessageBox.warning(self, core.APP_NAME, "Specify the path to cookies.txt.")
            return False
        if not self.download_dir_edit.text().strip():
            QMessageBox.warning(self, core.APP_NAME, "Specify the folder where downloaded files should be saved.")
            return False
        missing = core.describe_missing_dependencies(self.format_combo.currentText().strip() or "mp4")
        if missing:
            QMessageBox.critical(
                ...
                "Required components not found: "
                + ", ".join(missing)
                + ".\n\nInstall them first or place them next to the app.",
            )
            self.set_status("error")
            return False
        return True
    ...
    def choose_playlist_mode(self, url: str) -> str | None:
        ...
        box.setText("A video link from a playlist was detected.")
        box.setInformativeText("What should be downloaded: only the current video or the whole playlist?")
        single_button = box.addButton("Only this video", QMessageBox.AcceptRole)
        playlist_button = box.addButton("Whole playlist", QMessageBox.YesRole)
    ...
    def start_download(self):
        ...
        if playlist_mode is None:
            self.write_log("Download was cancelled by the user.")
            return
        ...
        def runner():
            ...
            try:
                ...
                self.bus.log.emit("Launching via PowerShell: " + ps_cmd)
                ...
                self.bus.error.emit(
                    ...
                    f"Check the path to yt-dlp, ffmpeg and PowerShell.\n\nTechnical details: {exc}"
                )
            finally:
    ...
    def stop_download(self):
        ...
        if not self.download_thread or not self.download_thread.is_alive():
            self.write_log("There is no active download.")
            self.stop_button.setEnabled(False)
            return
        self.set_status("stopping")
        self.stop_button.setEnabled(False)
        if not self.download_process or self.download_process.poll() is not None:
            self.write_log("Stop requested. Waiting for the process to start...")
            return
        self.write_log("Stopping download...")
        self._terminate_download_process()
        self.write_log("Stop command sent.")
